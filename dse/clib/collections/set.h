...
#ifndef DSE_CLIB_COLLECTIONS_SET_H_
#define DSE_CLIB_COLLECTIONS_SET_H_
/*******************************************************************************
***
***     Author: Tyler Barrus
...
/*  Initialize the set either with default parameters (hash function and space)
    ...
    Returns:
        ...
        SET_TRUE: On success
*/
DLL_PUBLIC int set_init_alt(
    SimpleSet* set, uint64_t num_els, set_hash_function hash);
...
DLL_PUBLIC int set_clear(SimpleSet* set);

/* Free all memory that is part of the set */
DLL_PUBLIC int set_destroy(SimpleSet* set);
...
*/
DLL_PUBLIC int set_add(SimpleSet* set, const char* key);
...
*/
DLL_PUBLIC int set_remove(SimpleSet* set, const char* key);
...
/*  Check if key in set
    ...
    NOTE: SET_CIRCULAR_ERROR should never happen, but is there for insurance!
*/
DLL_PUBLIC int set_contains(SimpleSet* set, const char* key);

/* Return the number of elements in the set */
DLL_PUBLIC uint64_t set_length(SimpleSet* set);
...
*/
DLL_PUBLIC int set_union(SimpleSet* res, SimpleSet* s1, SimpleSet* s2);
...
*/
DLL_PUBLIC int set_intersection(SimpleSet* res, SimpleSet* s1, SimpleSet* s2);
...
/*  Set res to the difference between s1 and s2
    ...
    for S ∖ T is S − T.
*/
DLL_PUBLIC int set_difference(SimpleSet* res, SimpleSet* s1, SimpleSet* s2);
...
/*  Set res to the symmetric difference between s1 and s2
    ...
    A △ B or A * B
*/
DLL_PUBLIC int set_symmetric_difference(
    SimpleSet* res, SimpleSet* s1, SimpleSet* s2);
...
*/
DLL_PUBLIC int set_is_subset(SimpleSet* test, SimpleSet* against);
...
/*  Strict subset ensures that the test is a subset of against, but that
    ...
    as A ⊂ B
*/
DLL_PUBLIC int set_is_subset_strict(SimpleSet* test, SimpleSet* against);
...
*/
static __inline__ int set_is_superset_strict(
    SimpleSet* test, SimpleSet* against)
{
...
DLL_PUBLIC char** set_to_array(SimpleSet* set, uint64_t* size);
...
*/
DLL_PUBLIC int set_cmp(SimpleSet* left, SimpleSet* right);
...
// UINT32 API
DLL_PUBLIC int set_add_uint32(SimpleSet* set, uint32_t key);
DLL_PUBLIC int set_contains_uint32(SimpleSet* set, uint32_t key);
DLL_PUBLIC int set_remove_uint32(SimpleSet* set, uint32_t key);
...
#endif  // DSE_CLIB_COLLECTIONS_SET_H_
